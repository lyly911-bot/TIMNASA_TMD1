...
timoth({ nomCom: "invite", categorie: 'Group', reaction: "🙋" }, async (dest, zk, commandeOptions) => {
...
timoth({ nomCom: "makeadmin", categorie: 'Group', reaction: "👨🏿‍💼" }, async (dest, zk, commandeOptions) => {
    ...
    if (autAdmin || superUser) {
      if (msgRepondu) {
        if (zkad) {
          if (membre) {
            if (admin == false) {
              var txt = `🎊you are now admin🎊🎊  @${auteurMsgRepondu.split("@")[0]} rose in rank.\n
...
timoth({ nomCom: "removeadmin", categorie: 'Group', reaction: "👨🏿‍💼" }, async (dest, zk, commandeOptions) => {
  let { repondre, msgRepondu, infosGroupe, auteurMsgRepondu, verifGroupe, auteurMessage, superUser, idBot } = commandeOptions;
  let membresGroupe = verifGroupe ? await infosGroupe.participants : ""
  if (!verifGroupe) { return repondre("For groups only"); }
...
/** ***fin démettre****  **/
/** **retirer** */
timoth({ nomCom: "gout", categorie: 'Group', reaction: "👨🏿‍💼" }, async (dest, zk, commandeOptions) => {
...
timoth({ nomCom: "delete", categorie: 'Group',reaction:"🧹" }, async (dest, zk, commandeOptions) => {
...
zokou({ nomCom: "info", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
 ...
 timoth({ nomCom: "antiprotect", categorie: 'Group', reaction: "🔗" }, async (dest, zk, commandeOptions) => {
  ...
  if( superUser || verifAdmin) {
    ...
    try {
      if (!arg || !arg[0] || arg === ' ') { repondre("antitimnasa on to activate the anti-timnasa feature\nantitimnasa off to deactivate the anti-timnasa feature\nantitimnasa action/remove to directly remove the timnasa without notice\nantitimnasa action/warn to give warnings\nantitimnasa action/delete to remove the timnasa without any sanctions\n\nPlease note that by default, the anti-timnasa feature is set to delete.") ; return};
     
      if(arg[0] === 'on') {

      
       if(enetatoui ) { repondre("the antitimnasa is already activated for this group")
                    } else {
                  await ajouterOuMettreAJourJid(dest,"oui");
                
              repondre("the antitimnasa is activated successfully") }
              ...
              if (enetatoui) { 
                ...
                repondre("The antitimnasa has been successfully deactivated");
                
              } else {
                repondre("antitimnasa is not activated for this group");
              }
              ...
              if ( action == 'remove' || action == 'warn' || action == 'delete' ) {
                ...
                repondre(`The anti-timnasa action has been updated to ${arg.join('').split("/")[1]}`);
            ...
            } else repondre("antitimnasa on to activate the anti-timnasa feature\nantitimnasa off to deactivate the anti-timnasa feature\nantitimnasa action/remove to directly remove the timnasa without notice\nantitimnasa action/warn to give warnings\nantitimnasa action/delete to remove the timnasa without any sanctions\n\nPlease note that by default, the anti-timnasa feature is set to delete.")
...
timoth({ nomCom: "leftbey", categorie: "Mods" }, async (dest, zk, commandeOptions) => {
...
timoth({ nomCom: "groupname", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
...
timoth({ nomCom: "groupdesc", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
...
timoth({ nomCom: "groupp", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
